...
--  COMMENT ON COLUMN Production.Document.DocumentLevel IS 'Depth in the document hierarchy.';
  ...
  COMMENT ON COLUMN Production.Document.FolderFlag IS 'FALSE = This is a folder, TRUE = This is a document.';
...
--  COMMENT ON COLUMN HumanResources.Employee.OrganizationLevel IS 'The depth of the employee in the corporate hierarchy.';
  ...
  COMMENT ON COLUMN HumanResources.Employee.SalariedFlag IS 'Job classification. FALSE = Hourly, not exempt from collective bargaining. TRUE = Salaried, exempt from collective bargaining.';
  ...
  COMMENT ON COLUMN HumanResources.Employee.CurrentFlag IS 'FALSE = Inactive, TRUE = Active';
...
COMMENT ON TABLE Person.Person IS 'Human beings involved with AdventureWorks: employees, customer contacts, and vendor contacts.';
  ...
  COMMENT ON COLUMN Person.Person.NameStyle IS 'FALSE = The data in FirstName and LastName are stored in western style (first name, last name) order. TRUE = Eastern style (last name, first name) order.';
...
COMMENT ON TABLE Production.Product IS 'Products sold or used in the manfacturing of sold products.';
  ...
  COMMENT ON COLUMN Production.Product.MakeFlag IS 'FALSE = Product is purchased, TRUE = Product is manufactured in-house.';
  COMMENT ON COLUMN Production.Product.FinishedGoodsFlag IS 'FALSE = Product is not a salable item. TRUE = Product is salable.';
...
COMMENT ON TABLE Production.ProductProductPhoto IS 'Cross-reference table mapping products and product photos.';
  ...
  COMMENT ON COLUMN Production.ProductProductPhoto.Primary IS 'FALSE = Photo is not the principal image. TRUE = Photo is the principal image.';
...
COMMENT ON TABLE Sales.SalesOrderHeader IS 'General sales order information.';
  ...
  COMMENT ON COLUMN Sales.SalesOrderHeader.OnlineOrderFlag IS 'FALSE = Order placed by sales person. TRUE = Order placed online by customer.';
...
COMMENT ON TABLE Person.StateProvince IS 'State and province lookup table.';
  ...
  COMMENT ON COLUMN Person.StateProvince.IsOnlyStateProvinceFlag IS 'FALSE = StateProvinceCode exists. TRUE = StateProvinceCode unavailable, using CountryRegionCode.';
...
COMMENT ON TABLE Purchasing.Vendor IS 'Companies from whom Adventure Works Cycles purchases parts or other goods.';
  ...
  COMMENT ON COLUMN Purchasing.Vendor.PreferredVendorStatus IS 'FALSE = Do not use if another vendor is available. TRUE = Preferred over other vendors supplying the same product.';
  COMMENT ON COLUMN Purchasing.Vendor.ActiveFlag IS 'FALSE = Vendor no longer used. TRUE = Vendor is actively used.';
